...
ถ้าลูกค้าสนใจจองใหม่ในวันพรุ่งนี้
ช่วงเวลาที่ว่างสำหรับการจองในวันที่ ${formatDate(cancellationDate)} มีดังนี้:
${availableTimes.length > 0 ? availableTimes.map(time => `- ${time}`).join('\n') : 'ไม่มีช่วงเวลาที่ว่างสำหรับวันนี้'}


หากสนใจจองใหม่ คลิกที่ลิงค์นี้ http://localhost:3000/AnimalClinic/BookingQueue
